...
(function(factory) {
	...
	var Kalendae = function(targetElement, options) {
		...
		var self = this,
			...
			opts = self.settings = util.merge(self.defaults, {
				attachTo: targetElement
			}, options || {}),
			$container = self.container = util.make('div', {
		...
		if (viewDelta && moment().month() == moment(self.viewStartDate).month()) {
			self.viewStartDate = moment(self.viewStartDate).subtract({
				M: viewDelta
			}).date(1);
		}
		...
		if (typeof opts.blackout === 'function') {
			...
			self.blackout = function(input) {
				...
				var i = bdates.length;
				while (i--)
					if (bdates[i].startOf('day').yearDay() === input) return true;
				return false;
			};
		} else {
		...
		while (j--) {
			...
			util.make('a', {
				'class': classes.previousYear
			}, $title); //previous button
			util.make('a', {
				'class': classes.previousMonth
			}, $title); //previous button
			util.make('a', {
				'class': classes.nextYear
			}, $title); //next button
			util.make('a', {
				'class': classes.nextMonth
			}, $title); //next button
			$caption = util.make('div', {
				'class': classes.caption
			}, $title); //title caption
			util.make('div', {
				'class': classes.captionMonth
			}, $caption); //month text
			util.make('div', {
				'class': classes.captionYear
			}, $caption); //year text

			//column headers
			$header = util.make('div', {
				'class': classes.header + ' ' + (opts.dayHeaderClickable == true ? classes.dayActive : '')
			}, $cal);
			i = 0;
			do {
				$span = util.make('span', {
				...
				if (opts.dayHeaderClickable == true && opts.mode == 'multiple') {
					$span.addEventListener("mouseover", function(e) {
						...
						if (daysToHover.length > 0) {
							for (var i = 0; i < daysToHover.length; i++) {
								if (util.hasClassName(daysToHover[i], classes.dayActive)) util.addClassName(daysToHover[i],
									'k-day-hover-active');
							}
						}
					});
					$span.addEventListener("mouseleave", function(e) {
						...
						if (daysToHover.length > 0) {
							for (var i = 0; i < daysToHover.length; i++) {
								if (util.hasClassName(daysToHover[i], classes.dayActive)) util.removeClassName(daysToHover[i],
									'k-day-hover-active');
							}
						}
					});
		...
		util.addEvent($container, 'mousedown', function(event, target) {
			...
			} else if ((util.hasClassName(target.parentNode, classes.days) || util.hasClassName(target.parentNode, classes.week)) &&
				util.hasClassName(target, classes.dayActive) && (clickedDate = target.getAttribute('data-date'))) {
				//DAY CLICK
...
// 	var colorClass = "";
// 	if (hour >= 0 && hour < 5)
// 		colorClass = "k-selected2";
// 	else(hour >= 19 && hour <= 24)
// 	colorClass = "k-selected2";
// 	else
// 		colorClass = "k-selected";

	Kalendae.prototype = {
		defaults: {
			attachTo: null,
			/* the element to attach the root container to. can be string or DOMElement */
			months: 1,
			/* total number of months to display side by side */
			weekStart: 0,
			/* day to use for the start of the week. 0 is Sunday */
			direction: 'any',
			/* past, today-past, any, today-future, future */
			directionScrolling: true,
			/* if a direction other than any is defined, prevent scrolling out of range */
			viewStartDate: null,
			/* date in the month to display.  When multiple months, this is the left most */
			endDate: null,
			/* date calendar cannot scroll or select past */
			blackout: null,
			/* array of dates, or function to be passed a date */
			selected: null,
			/* dates already selected.  can be string, date, or array of strings or dates. */
			mode: 'single',
			/* single, multiple, range */
			dayOutOfMonthClickable: false,
			dayHeaderClickable: false,
			format: null,
			/* string used for parsing dates. */
			subscribe: null,
			/* object containing events to subscribe to */

			columnHeaderFormat: 'dd',
			/* number of characters to show in the column headers */
			titleMonthFormat: 'MMMM,',
			/* format mask for month titles. */
			titleYearFormat: 'YYYY',
			/* format mask for year titles. */
			dayNumberFormat: 'D',
			/* format mask for individual days */
			dayAttributeFormat: 'YYYY-MM-DD',
			/* format mask for the data-date attribute set on every span */
			parseSplitDelimiter: /,\s*|\s+-\s+/,
			/* regex to use for splitting multiple dates from a passed string */
			rangeDelimiter: ' - ',
			/* string to use between dates when outputting in range mode */
			multipleDelimiter: ', ',
			/* string to use between dates when outputting in multiple mode */
			useYearNav: true,
		...
		draw: function draw() {
			...
			do {
				...
				do {
					...
					if (s) klass.push(({
						'-1': classes.dayInRange,
						'1': classes.daySelected,
						'true': classes.daySelected
					})[s]);
					...
					if (!(this.blackout(day) || this.direction(day) || (!!this.endDate && day.isAfter(this.endDate, 'day')) || (
							day.month() != month.month() && opts.dayOutOfMonthClickable === false)) || s > 0)
	...
	var util = Kalendae.util = {
		...
		isIE8: function() {
			return !!((/msie 8./i).test(navigator.appVersion) && !(/opera/i).test(navigator.userAgent) && window.ActiveXObject &&
		...
		make: function(tagName, attributes, attach) {
			...
			if (!!attributes)
				for (k in attributes)
					if (attributes.hasOwnProperty(k)) e.setAttribute(k, attributes[k]);
		...
		hasClassName: function(elem, className) { //copied and modified from Prototype.js
			...
			return (eClassName.length > 0 && (eClassName == className || new RegExp("(^|\\s)" + className + "(\\s|$)").test(
				eClassName)));
		},
	...
	if (typeof document.addEventListener === 'function') Kalendae.util.domReady(function() {
		...
		while (i--) {
			...
			if (e.tagName === 'INPUT') {
				...
				new Kalendae(util.merge(options, {
					attachTo: e
				}));
			}
		...
		jQuery.fn.kalendae = function(options) {
			this.each(function(i, e) {
				if (e.tagName === 'INPUT') {
					...
					//otherwise, insert a flat calendar into the element.
					jQuery(e).data('kalendae', new Kalendae(jQuery.extend({}, {
						attachTo: e
					}, options)));
				}
			});
